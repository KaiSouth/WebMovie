               ...
               <div style="margin-top:10px;">
                  ...
                  <div class="col-md-4 hidden-xs" style="text-align: center;">
                    @if(Auth::guard('customer')->check())
                        <p>Xin chào, {{ Auth::guard('customer')->user()->name }}</p>
                        <!-- Thêm form đăng xuất -->
                        <form action="/logoutCustomer" method="POST">
                            @csrf <!-- Token bảo vệ CSRF -->
                            <button type="submit" style="background-color: #f44336; color: #FFF; padding: 5px 10px; border-radius: 5px; border: none; cursor: pointer;">Đăng Xuất</button>
                        </form>
                    @else
                        <button class="login-btn" onclick="window.location.href='/loginCus';" style="background-color: rgba(0, 152, 210, 0.07); color: #FFF; padding: 10px 20px; border-radius: 5px; border: none; cursor: pointer;">Đăng Nhập</button>
                    @endif
                </div>
